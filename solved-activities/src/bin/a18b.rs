...
fn authorize(
    employee_name: &str,
    employee_location: ProtectedLocation,
) -> Result<AuthorizationStatus, String> {
    let user_database: Database = Database::connect()?;
    let employee = user_database.find_employee(employee_name)?;
    let employee_keycard = user_database.get_keycard(&employee)?;

    let access_level = employee_location.required_access_level();

    if employee_keycard.access_level >= access_level {
